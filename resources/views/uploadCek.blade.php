	...
	<div class="row">
		<div class="container">
			...
			@if(count($errors) > 0)
				...
				<form action="{{url('upload/proses/'.$data->id)}}" method="POST" enctype="multipart/form-data" onsubmit="alert('upload success');">
					@csrf
				...
				<form action="{{url('updatelunas/'.$data->id)}}" method="POST" onsubmit="alert('Pesanan Lunas.');">
				@csrf
